...
class Obstacles:
    def __init__(self, obs: dict, dt: float):
        ...
        # dont know why but second obs velocity is wrong way
...
class Obstacle:
    ...
    def collides_at_time_step(self, pos: Tuple[float, float, float], time_step: int, safety_margin: float) -> bool:
        ...
        # print("--obs_pos:", self.pos, "  f_obs_pos:",f_obs_pos)
        # gripper dimension
        g_x_width = 0.03
        g_y_depth = 0.01
        g_z_up = 0.0
        ...
        if not (pos[2] + g_z_up + safety_margin >= f_obs_pos[2] - self.size[2] and
            ...
            return False
        # print("--collides: pos:",pos, "f_obs_pos:",f_obs_pos, "size:",self.size )
        return True
    ...
    def _future_obstacle_positions(self) -> Tuple[float, float, float]:
        ...
        for i in range(20):
            new_pos = [0, 0, 0]
            # print("--obs_pos:", self.pos, self.direction , i , self.dt , self.vel)
            for i in range(3):
...
class FindTrajectory:
    ...
    def is_nearest_node_to_goal(self, node: Node) -> bool:
        ...
        # old:
        # for i in range(3):
        #     if abs(node.pos[i] - self.goal_pos[i]) > (self.step_size -0.015):
        #         return False
        # return True
    ...
    def a_star_search(self):
        ...
        while len(open_dic) > 0:
            ...
            # if current_node.__eq__(start_node) and children is None:
            #     print("start node has no possebility to move")
