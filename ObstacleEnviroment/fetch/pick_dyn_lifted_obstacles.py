...
MODEL_XML_PATH = os.path.join(os.getcwd(), 'ObstacleEnviroment', 'assets', 'fetch', 'pick_dyn_lifted_obstacles.xml')
...
def pretty_obs(obs):
    return {'gripper_pos': obs[0:5], 'obj_to_grasp': obs[5:8], 'obstacle1': obs[8:15],
            'obstacle2': obs[15:22], 'obstacle3': obs[22:29], "goal": obs[29:32]}


class FetchPickDynLiftedObstaclesEnv(robot_env.RobotEnv, gym.utils.EzPickle):
    # todo: find right n_substeps
    def __init__(self, reward_type='sparse', n_substeps=100):
        ...
        self.current_obstacle_vels = [[0], [0]]

        super(FetchPickDynLiftedObstaclesEnv, self).__init__(
    ...
    def _setup_dyn_obstacles(self):
        ...
        # self.vel_lims = np.array([0.6, 0.9]) #original
        self.vel_lims = np.array([0.0, 0.0])
        # self.vel_lims2 = np.array([0.2, 0.6])#original
        self.vel_lims2 = np.array([0.0, 0.0])
    ...
    def step(self, action):
        ...
        return super(FetchPickDynLiftedObstaclesEnv, self).step(action)
    ...
    def _get_obs(self):
        ...
        # print("grip_pos:", grip_pos," gripper_state:",gripper_state)
        # print("self.current_obstacle_vels[0]",self.current_obstacle_vels[0])
        obs = np.concatenate([
            grip_pos, gripper_state, object_pos.ravel(), ob1, self.current_obstacle_vels[0]
            , ob2, self.current_obstacle_vels[1] , self.stat_obstacles[0], [0],self.goal.copy()
        ])
        # --------change obs:-------
    ...
    def render(self, mode='human', width=1080, height=1080):
        return super(FetchPickDynLiftedObstaclesEnv, self).render(mode, width, height)
