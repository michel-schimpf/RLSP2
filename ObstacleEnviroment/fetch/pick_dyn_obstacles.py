...
MODEL_XML_PATH = os.path.join(os.getcwd(), 'ObstacleEnviroment', 'assets', 'fetch', 'pick_dyn_obstacles.xml')
...
def pretty_obs(obs):
    return {'gripper_pos': obs[0:5], 'obj_to_grasp': obs[5:8], 'obstacle1': obs[8:15],
            'obstacle2': obs[15:22], "goal": obs[29:32]}


class FetchPickDynObstaclesEnv(robot_env.RobotEnv, gym.utils.EzPickle):
    def __init__(self, reward_type='sparse', n_substeps=20):
        ...
        super(FetchPickDynObstaclesEnv, self).__init__(
    ...
    def step(self, action):
        ...
        return super(FetchPickDynObstaclesEnv, self).step(action)
    ...
    def render(self, mode='human', width=1080, height=1080):
        return super(FetchPickDynObstaclesEnv, self).render(mode, width, height)
